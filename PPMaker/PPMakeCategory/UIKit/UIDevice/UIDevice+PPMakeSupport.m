 ...
 * 可是自从有了iPhone X系列，苹果固定底部有34不能用来响应事件，而是用来处理"Home键"的事件(当然了tableView列表型的可以除外)，so,现在需要这样用：
...
{
    return k_isiPhoneXSeries ? 44 : 20;
}
/** 导航栏高度 */
+ (CGFloat)ppmake_navBarHeight
{
    return k_isiPhoneXSeries ? 88 : 64;
}
@end
